...
description: VSDM形式からVSTXファイルへのサンプルJava変換コード。このサンプルコードを使用して、WebまたはデスクトップJavaベースのアプリケーション内でVSDMをVSTXに変換します。
---
...
1. Diagramクラスのインスタンスを含むVSDMファイルをロードします1. 出力ファイルパスとSaveFileFormatをパラメーターとして使用してDiagram.saveメソッドを呼び出します1. VSTXファイルは指定されたパスに保存されます
...
{{< blocks/products/pf/agp/demobox sectionTitle="VSDMからVSTXへの変換ライブデモ" sectionDescription="[VSDMをVSTXに変換する](https://products.aspose.app/diagram/conversion/vsdm-to-vstx) 今すぐライブデモのウェブサイトにアクセスしてください。ライブデモには次の利点があります。" >}}
