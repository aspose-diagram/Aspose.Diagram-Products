...
description: VSDM形式からVSDXファイルへのJava変換コードのサンプル。このサンプルコードを使用して、WebまたはデスクトップJavaベースのアプリケーション内でVSDMをVSDXに変換します。
---
...
1. Diagramクラスのインスタンスを含むVSDMファイルをロードします1. 出力ファイルパスとSaveFileFormatをパラメーターとしてDiagram.saveメソッドを呼び出します1. VSDXファイルは指定されたパスに保存されます
...
{{< blocks/products/pf/agp/demobox sectionTitle="VSDMからVSDXへの変換ライブデモ" sectionDescription="[VSDMをVSDXに変換](https://products.aspose.app/diagram/conversion/vsdm-to-vsdx) 今すぐライブデモのウェブサイトにアクセスしてください。ライブデモには次の利点があります。" >}}
