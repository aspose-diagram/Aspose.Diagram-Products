...
description: VSDM形式からVSTMファイルへのJava変換コードのサンプル。このサンプルコードを使用して、WebまたはデスクトップJavaベースのアプリケーション内でVSDMをVSTMに変換します。
---
...
1. Diagramクラスのインスタンスを含むVSDMファイルをロードします1. 出力ファイルパスとSaveFileFormatをパラメーターとしてDiagram.saveメソッドを呼び出します1. VSTMファイルは指定されたパスに保存されます
...
{{< blocks/products/pf/agp/demobox sectionTitle="VSDMからVSTMへの変換ライブデモ" sectionDescription="[VSDMをVSTMに変換](https://products.aspose.app/diagram/conversion/vsdm-to-vstm) 今すぐライブデモのウェブサイトにアクセスしてください。ライブデモには次の利点があります。" >}}
