...
description: VSSM形式のVSDXファイルへのJava変換コードのサンプル。このサンプルコードを使用して、WebまたはデスクトップJavaベースのアプリケーション内でVSSMをVSDXに変換します。
---
...
1. Diagramクラスのインスタンスを含むVSSMファイルをロードします1. 出力ファイルパスとSaveFileFormatをパラメーターとして使用してDiagram.saveメソッドを呼び出します1. VSDXファイルは指定されたパスに保存されます
...
{{< blocks/products/pf/agp/demobox sectionTitle="VSSMからVSDXへの変換ライブデモ" sectionDescription="[VSSMをVSDXに変換します](https://products.aspose.app/diagram/conversion/vssm-to-vsdx) 今すぐライブデモのウェブサイトにアクセスしてください。ライブデモには次の利点があります。" >}}
