...
{{% blocks/products/pf/agp/content h2="C#를 사용하여 VSDM을 VSTX로 변환하는 방법" %}}

 VSDM을(를) VSTX(으)로 변환하기 위해 다음을 사용합니다.
...
{{% blocks/products/pf/agp/feature-section-col title="C#를 통해 VSDM을 VSTX로 변환하는 단계" %}}
...
1. Diagram 클래스의 인스턴스로 VSDM 파일 로드1. 출력 파일 경로 및 SaveFileFormat.VSTX을 매개변수로 사용하여 Diagram.Save 메소드를 호출합니다.1. VSTX 파일이 지정된 경로에 저장됩니다.
...
{{% blocks/products/pf/agp/code-block title="이 샘플 코드는 VSDM에서 VSTX로의 C# 전환을 보여줍니다." offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="VSDM을(를) VSTX(으)로 변환하는 무료 앱" sectionDescription="라이브 데모를 확인하십시오. [VSDM에서 VSTX로의 전환](https://products.aspose.app/diagram/conversion/vsdm-to-vstx) 다음과 같은 혜택이 있습니다." >}}
