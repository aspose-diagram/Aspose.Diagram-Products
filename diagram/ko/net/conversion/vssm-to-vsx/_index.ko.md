...
description: VSSM에서 VSX로의 C# 변환을 위한 샘플 코드입니다. VB.NET, Asp.NET 또는 모든 .NET 기반 애플리케이션 내에서 일괄 VSSM 파일을 VSX로 변환하는 API 예제 코드를 사용합니다.
---
...
{{% blocks/products/pf/agp/content h2="C#을 사용하여 VSSM을 VSX로 변환하는 방법" %}}

 VSSM을 VSX로 변환하려면 다음을 사용합니다.
...
{{% blocks/products/pf/agp/feature-section-col title="C#을 통해 VSSM을 VSX로 변환하는 단계" %}}
...
1. Diagram 클래스의 인스턴스로 VSSM 파일 로드1. 출력 파일 경로와 SaveFileFormat.VSX를 매개변수로 사용하여 Diagram.Save 메소드를 호출합니다.1. VSX 파일은 지정된 경로에 저장됩니다.
...
{{% blocks/products/pf/agp/code-block title="이 샘플 코드는 VSSM에서 VSX로의 C# 변환을 보여줍니다." offSpacer="" %}}
...
{{< blocks/products/pf/agp/demobox sectionTitle="VSSM을 VSX로 변환하는 무료 앱" sectionDescription="라이브 데모를 확인하십시오. [VSSM에서 VSX로의 변환](https://products.aspose.app/diagram/conversion/vssm-to-vsx) 다음과 같은 혜택이 있습니다." >}}
