...
{{% blocks/products/pf/agp/feature-section-col title="Python을(를) 통해 VSDX을(를) 만드는 방법" %}}

{{% blocks/products/pf/agp/text %}}

 개발자는 단 몇 줄의 코드로 데이터 처리를 위해 다양한 보고 응용 프로그램을 실행하여 VSDX(Portable Document Format)을 쉽게 생성, 로드, 수정 및 변환할 수 있습니다.

{{% /blocks/products/pf/agp/text %}}

1. 클래스 파일에 네임스페이스 포함1. Diagram 클래스 인스턴스를 만듭니다.1. diagram의 첫 번째 페이지에 액세스합니다.1. 페이지에 텍스트를 추가합니다.1. 저장 방법을 사용하여 diagram을 VSDX 파일로 저장합니다.
...
<!-- aboutfile Starts -->
 ...
 VSDX 파일을 생성, 수정, 변환, 렌더링 및 인쇄할 수 있는 기능으로 플랫폼 간 애플리케이션을 빌드할 수 있는 Visio 프로그래밍 라이브러리입니다. .NET Visio API 스프레드시트 형식 간에 변환할 뿐만 아니라 Visio 파일을 이미지, VSDX, VSDX 등으로 렌더링할 수 있으므로 업계 표준에서 문서를 교환하는 데 완벽한 선택입니다. 형식.
